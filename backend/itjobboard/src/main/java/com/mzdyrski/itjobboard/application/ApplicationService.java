...
public class ApplicationService {
    ...
    public ApplicationState checkIfCanApply(User user, String offerId) {
        if (Objects.equals(user.getRole(), ROLE_EMPLOYEE.name())) {
            ...
            var cv = mongoTemplate.findOne(new Query(new Criteria("employeeId").is(user.getId())), EmployeesCv.class, "employees_cvs");
            if (cv == null) {
                return NO_CV;
            } else if (app == null) {
                return CAN_APPLY;
    ...
    private void sendEmailsAfterApplying(User employee, String offerId) throws MessagingException {
        ...
        var cv = mongoTemplate.findOne(new Query(new Criteria("employeeId").is(employee.getId())), EmployeesCv.class, "employees_cvs");
